...
\item[decomposable community] is a community with no more than 30 pages. It can be decomposed to that the members are displayed.
...
\item[final samples] are the third and final learning data set base on enhanced samples with 9 categories
\item[first samples] are the learning first data set containing 13 categories
\item[igraph] the python-igraph library
\item[mac count] it the maximum desire number of communities to be returned by the API
...
The attachments enclosed in the thesis repository\footnote{https://is.muni.cz/auth/th/noh49/} contain three source codes with and dumps. The files have the following structure.
\begin{description}
